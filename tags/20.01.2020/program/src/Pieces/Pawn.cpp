...
bool Pawn::can_move(std::shared_ptr<Field> field_final) { // board row count starts at 0 ends at 7. Black pawns start at row 6, white at row 1. (Player sees the pieces on fields 1-8)
    Position tmp = field_final->get_position();
    if (this->get_field()->get_position().operator==(tmp)) {
        return false;
    }
    if (this->get_is_white()){
        if(field_final->get_piece() == nullptr) {
